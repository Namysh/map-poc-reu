...
const show = (map, CLUSTERS, loading) => {
    ...
    const fakeFeatures = Array.from({length: 2000000}, () => ({
        type: 'Feature',
        geometry: {
            type: 'Point',
            coordinates: [
    ...
    map.addSource('init-clusters-source', {
        type: 'geojson',
        // data: 'https://docs.mapbox.com/mapbox-gl-js/assets/init-clusters-source.geojson',
        data: {
    ...
    sources.push('init-clusters-source')

    map.addLayer({
        id: 'init-clusters',
        type: 'symbol',
        source: 'init-clusters-source',
    ...
    layers.push('init-clusters')


    map.addLayer({
        id: 'init-cluster-count',
        type: 'symbol',
        source: 'init-clusters-source',
        filter: ['has', 'point_count'],
        layout: {
    ...
    layers.push('init-cluster-count')
    ...
    const handleCluterClick = (e) => {
        ...
        map.getSource('init-clusters-source').getClusterExpansionZoom(
            clusterId,
            (err, zoom) => {
                if (err) return;
    ...
    //
    // map.addLayer({
    //     id: 'real-clusters-layer',
    //     type: 'symbol',
    //     source: 'real-clusters',
    //     filter: ['has', 'point_count'],
    //     layout: {
    //         'icon-image': [
    //             'step',
    ...
    map.on('sourcedata', (r) => {
        if (r.sourceId === 'init-clusters-source') {
            if (r.isSourceLoaded) {
        ...
        // if (r.sourceId === 'real-clusters') {
        //     if (r.isSourceLoaded) {
        //         if (realLoaded.value) return
        //
        //         realLoaded = true
        //         console.log('real clusters loaded')
        //         // map.removeSource('ear')
        //         map.removeLayer('init-clusters')
        //         map.removeLayer('init-clusters-count')
        //     }
        // }
    })
